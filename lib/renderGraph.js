...
const text = (xw, pt, label) => {
    if (label) {
        xw.startElement("text");
            ...
            xw.writeAttribute("textLength", pt.width-8);
...
const use = (xw, pt) => {
    xw.startElement("use");
        xw.writeAttribute("xlink:href", "#"+shape.get(pt.type));
        xw.writeAttribute("x", pt.x-(pt.width/2)).writeAttribute("y", pt.y-(pt.height/2));
    xw.endElement();
}
